...
export const getOnlineUsers = async (req, res) => {
    try {
        const getOnlineUsers = await User.find({online:true});
        const getAllUsers = await User.find();
        // console.log(getOnlineUsers,getAllUsers); 
        return res.status(200).json({ online: getOnlineUsers,allUsers: getAllUsers,success: true });
    } catch (error) {
        console.log("🚀 ~ file: chat.js:13 ~ get Online Users ~ error:", error)
        return res.status(500).json({ success: false, message: 'Something went wrong' })
    }
}

export const getChartWeek = async (req, res) => {
    try {
        // const getOnlineUsers = await User.find({online:true});
        const weekChart = await Chat.aggregate([
            {
              $project: {
                week: { $week: "$createdAt" } 
              }
        ...
        return res.status(200).json({ data:weekChart ,success: true });
...
export const getChartDay = async (req, res) => {
    try {
        // const getOnlineUsers = await User.find({online:true});
        const dayChart = await Chat.aggregate([
            {
              $project: {
        ...
        return res.status(200).json({ data:dayChart ,success: true });
...
export const getGrowth = async (req, res) => {
    try {
        const getGrowth = await User.aggregate([
            {
              $project: {
                dayOfYear: { $dayOfYear: "$createdAt" } 
              }
        ...
        //   console.log('growth',getGrowth);
        return res.status(200).json({ data:getGrowth ,success: true });
